...
const CartSummary = ({ totals, currentUserAuth }) => {
  const navigate = useNavigate();
  ...
  const handleCheckout = async () => {
    ...
    if (totals.totalPrice < 1) {
      alert(`Order amount is too low: ₹${totals.totalPrice}. Minimum order value is ₹1. Please check your cart calculations or add more items.`);
      return;
    }

    // Temporary fix: If amount is suspiciously low, use a test amount
    let finalAmount = totals.totalPrice;
    if (totals.totalPrice < 10) {
      finalAmount = 100; // ₹100 for testing
      if (!confirm(`Cart total seems low (₹${totals.totalPrice}). Use test amount ₹${finalAmount} for demo?`)) {
        return;
      }
    }
    ...
    try {
      ...
      script.onload = () => {
        ...
        const options = {
          ...
          currency: 'INR',
          name: 'Rodella Composites Shop',
          description: `Order for ${totals.totalItems} items`,
          image: '/assets/logo.jpg', // Your logo (fixed path)
            ...
            try {
              ...
              const orderData = {
                ...
                items: cartItems.map(item => ({
                  ...
                  isCompositePlate: item.product.isCompositePlate || false,
                  isTubeProduct: item.product.isTubeProduct || false,
                  isReinforcementProduct: item.product.isReinforcementProduct || false,
                  isCoreProduct: item.product.isCoreProduct || false,
                  isEpoxyProduct: item.product.isEpoxyProduct || false
                                 })),
                 total: finalAmount, // Use the final amount (either original or test amount)
                 originalTotal: totals.totalPrice, // Keep original for reference
                 totalItems: totals.totalItems,
                totalWeight: totals.totalWeight,
                totalArea: totals.totalArea,
                                 paymentMethod: 'Razorpay',
                 paymentStatus: 'Completed',
                 status: 'Processing', // This is what UserDashboard expects
                customerInfo: {
                  name: userInfo.name,
              ...
              if (orderId) {
                alert('Payment successful! Your order has been placed. Order ID: ' + orderId.slice(-8).toUpperCase());
                clearCart();
              ...
              alert('Payment successful but there was an issue saving your order. Please contact support with payment ID: ' + response.razorpay_payment_id);
        ...
        {!cartLoaded ? (
          ...
          <div className="space-y-6">
            ...
            <div className="lg:col-span-1">
              <CartSummary totals={totals} currentUserAuth={currentUser} />
            </div>
            </div>
          </div>
        )}
