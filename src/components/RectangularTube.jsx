...
const TubeLengthSVG = ({ length, size, wall, unit }) => {
  const dimensions = tubeSizes[size] || tubeSizes['18x28'];
  const { width, height } = dimensions;
  const svgHeight = 300;
  ...
  // Calculate additional wall thickness beyond base 1mm
  const additionalWall = Math.max(0, wall - 1);
  const tubeHeight = (height + additionalWall * 2) * 2;
    ...
    >
      <svg width={maxWidth} height={svgHeight} viewBox={`0 0 ${maxWidth} ${svgHeight}`} className="min-w-[300px]">
        ...
        <motion.line
          ...
          x2={centerX - 30}
          y2={(svgHeight - tubeHeight) / 2 + additionalWall * 2}
          stroke="#a3a3a3"
          strokeWidth="2"
          strokeDasharray="5,5"
        />
        ...
        <motion.text
          ...
          transition={{ duration: 0.3, delay: 0.5 }}
          x={centerX - 45}
          y={(svgHeight - tubeHeight) / 2 + additionalWall}
          textAnchor="middle"
          fill="#a3a3a3"
          fontSize="16"
          fontWeight="bold"
          
          transform={`rotate(-90 ${centerX - 45} ${(svgHeight - tubeHeight) / 2 + additionalWall})`}
        >
          Wall: {wall} {unit}
        </motion.text>
...
const RectangularTube = () => {
  const [tube, setTube] = useState({
    ...defaultTube,
    size: '18x28', // Ensure size is set in initial state
  ...
  const handleAddToCart = () => {
    ...
    const product = {
      ...
      images: ['https://via.placeholder.com/300x200?text=Rectangular+Tube'], // Add actual image URL
      details: {
        ...
        >
          ...
          <div className="formfield flex flex-wrap gap-2 mb-4 sm:mb-8 justify-center">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option value="18x28">18 x 28 mm</option>
                <option value="23.4x36">23.4 x 36 mm</option>
                <option value="65x75">65 x 75 mm</option>
                <option value="76x116">76 x 116 mm</option>
              </select>
            </div>
          </div>
